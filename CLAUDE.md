...
`os.OpenRoot` belongs only in `internal/rootfs`. Do not add a generic `Create`/`OpenFile` that truncates before the caller can establish identity, an accessor returning an authorized absolute pathname, or cleanup that can delete a replacement object owned by another writer — meaning: exclude every writer this codebase itself could run concurrently, with a lock keyed by physical identity, never with a check that only narrows the window. The one narrow exception is a second writer genuinely outside this process, with independent access to the same directory, actively racing the exact syscall: no portable primitive in Go's stdlib binds a rename or unlink to a specific already-open handle rather than a name, on any OS this project targets, so a stat-then-act check immediately before the syscall (`os.SameFile`) is the accepted mitigation there, not a substitute for excluding an internal writer. This is the same boundary `internal/git`'s `repoMutationLocks` already draws for its own mutation lock ("guards harness-internal concurrency only... a user running git in the same repository at the same moment is outside its reach") — see `internal/rootfs.WriteStreamAtomic`'s doc comment for the concrete case.
