...
// PartType names the content shape of a message part.
const (
	PartText       = "text"
...
// MessagePart is a single content part within a conversation turn.
// A single assistant message may contain text + multiple tool_calls,
// and each tool call gets a matching tool_result part.
...
// LoopMessage is a conversation turn with part-based content, designed
// for UI display and session replay. The Parts slice preserves tool
// call boundaries that ChatMessage (role+content) loses.
type LoopMessage struct {
	Role  string        `json:"role"`
	Parts []MessagePart `json:"parts"`
}

// EventToParts converts a list of loop events back into message parts
// suitable for persistence and replay.
func EventToParts(events []Event) []LoopMessage {
	var msgs []LoopMessage
	var cur *LoopMessage
	for _, ev := range events {
		switch ev.Type {
		case EvtText:
			if cur == nil || cur.Role != "assistant" {
				cur = &LoopMessage{Role: "assistant"}
				msgs = append(msgs, *cur)
				cur = &msgs[len(msgs)-1]
			}
			cur.Parts = append(cur.Parts, MessagePart{
				Type:    PartText,
				Content: ev.Content,
			})
		case EvtToolCall:
		...
		case EvtDone, EvtLimit, EvtDoom, EvtCancel, EvtError:
			// Terminal events are not part of the conversation.
		}
	}
	return msgs
}
