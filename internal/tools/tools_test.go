...
func TestRegistry_ListAndGet(t *testing.T) {
	...
	all := r.List()
	if len(all) != 12 {
		t.Fatalf("expected 12 tools, got %d", len(all))
	}
	for _, id := range []string{"read", "file_list", "ast_map", "ast_find", "git_status", "git_diff", "git_log", "edit", "exec", "go_test", "go_lint", "web_search"} {
		if r.Get(id) == nil {
